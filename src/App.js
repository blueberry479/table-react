...
function App() {
  ...
  useEffect(() => {
    fetch(
      `http://filltext.com/?rows=${dataSize}&id={number|1000}&fname={firstName}&lname={lastName}&pretty=true&email={email}&phone={phone|(xxx) xxx-xx-xx}&streetAddress={streetAddress}&city={city}&company={business}&state={usState|abbr}`
    )
  ...
  const getFilteredData = () => {
    ...
    return data.filter((item) => {
      return (
        item["fname"].toLowerCase().includes(search.toLocaleLowerCase()) ||
  ...
  const currentItem = Array.from(filteredData).slice(
    firstItemIndex,
    lastItemIndex
  );
  return (
